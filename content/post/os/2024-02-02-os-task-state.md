...
### 3.3、任务初始化
现在有了空闲进程和初始化进程，就不再需要之前测试的进程了。需要多着链各个基础任务初始化，如下：
```cpp
void task_init()
{
...
```
如上所示，空闲进程属于内核进程，任务优先级较低为1。init进程为用户进程，任务优先级为5。这里晕车会在idle和init进程间来回切换，但是init执行频率更高，因为它的优先级更高。如果给init进程增加了阻塞，那就会一直运行idle进程。
...
3. 调度到了text进程也打印一次，进入休眠。此时两个进程都开始休眠，运行idle线程。
