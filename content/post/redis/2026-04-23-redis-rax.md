...
### 3.3、返回栈顶元素
返回栈顶元素和出栈实现比较类似，不同的是，它只返回元素，但是不会实际的操作栈顶。如下所示：
```c
...
### 3.4、释放栈
当内置数组不够时，我们需要申请堆内存来存放栈的数据，那在释放时这些内存也是需要释放的，代码如下：
```c
...
### 4.2、查询元素
这里我们先介绍查询，是因为在插入元素时也需要先做查询操作，找到新元素应该插入的位置。rax提供了查找key的接口raxFind，该接口用于获取key对应的value值，定义如下：
```c
